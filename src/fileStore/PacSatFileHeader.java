...
public class PacSatFileHeader implements Comparable<PacSatFileHeader>, Serializable{
	...
	public short headerChecksumValid() {
		...
		// Grab all the bytes except Header Checksum itself
		int h = 2;
		for (PacSatField f : fields) {
...
//		Log.infoDialog("CHECKSUMS", "PFH: " + headerChecksum.getLongValue() + "\nCALC: "+ bodyCS);
	...
	*/
	...
	public String[] getTableFields() {
		...
		fields[0] = getFieldById(FILE_ID).getLongString(); //.getLongHexString();
		...
		if (getFieldById(PacSatFileHeader.FILE_TYPE) != null) {
		PacSatField typeField = getFieldById(PacSatFileHeader.FILE_TYPE);
		int t = (int) typeField.getLongValue();
		String s = getTypeStringById(t);
		if (s == null) s = "Unknown";
		fields[11] = s + "-" + t;
		} else
			fields[11] = "";
		...
		if (getFieldById(COMPRESSION_TYPE) != null) {
			...
			if (compressedBy > 0 && compressedBy < compressions.length) 
...
//		s = s + " MOD: " + getFieldById(LAST_MOD_TIME).getDateValue();
		...
		s = s + " BCRC: " + getFieldById(BODY_CHECKSUM).getLongHexString();
		if (getFieldById(HEADER_CHECKSUM) != null)
			s = s + " HCRC: " + getFieldById(HEADER_CHECKSUM).getLongHexString();
		if (getFieldById(COMPRESSION_TYPE) != null) {
			int compressedBy =  (int) getFieldById(COMPRESSION_TYPE).getLongValue();
			s = s + " Compression: " +compressedBy + " " + compressions[compressedBy];
		}
	...
	public static void main(String[] args) throws IOException, FrameException, MalformedPfhException {
		...
		*/
		...
		int[] byheader = {0xAA,0x55,0x01,0x00,0x04,0x00,0x00,0x00,0x00,0x02,0x00,0x08,0x42,0x44,0x43,0x41,
				0x53,0x54,0x55,0x31,0x03,0x00,0x03,0x54,0x58,0x54,0x04,0x00,0x04,0xF8,0x07,0x00,0x00,0x05,
				0x00,0x04,0xC4,0x94,0x7C,0x5E,0x06,0x00,0x04,0xFE,0x3D,0x74,0x28,0x07,0x00,0x01,0x00,0x08,
				0x00,0x01,0x00,0x09,0x00,0x02,0x77,0x19,0x0A,0x00,0x02,0x1A,0x0A,0x0B,0x00,0x02,0x49,0x00,
				0x00,0x00,0x00};
		...
		int[] header_no_checksum = {0xAA,0x55,0x01,0x00,0x04,0x00,0x00,0x00,0x00,0x02,0x00,0x08,0x42,0x44,0x43,0x41,
				0x53,0x54,0x55,0x31,0x03,0x00,0x03,0x54,0x58,0x54,0x04,0x00,0x04,0xF8,0x07,0x00,0x00,0x05,
				0x00,0x04,0xC4,0x94,0x7C,0x5E,0x06,0x00,0x04,0xFE,0x3D,0x74,0x28,0x07,0x00,0x01,0x00,0x08,
				0x00,0x01,0x00,0x09,0x00,0x02,0x77,0x19,0x0A,0x00,0x02,0x00,0x00,0x0B,0x00,0x02,0x49,0x00,
				0x00,0x00,0x00};
		
		short result = 0;
		for (int i : header_no_checksum)
			result += i;
